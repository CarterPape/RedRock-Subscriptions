...
function memberful_sign_out_url() {
    return memberfulSiteURL('auth/sign_out');
}
...
function memberful_account_url($format = MEMBERFUL_HTML) {
    return memberfulSiteURL('account', $format);
}

function memberful_registration_page_url() {
    return memberfulSiteURL('register');
}

function memberful_account_get_download_url($download_slug) {
    return memberfulSiteURL('account/downloads/get/'.memberful_wp_extract_id_from_slug($download_slug));
}

function memberful_admin_member_url(
    $member_id,
    $format = MEMBERFUL_HTML
)
{
    return memberfulSiteURL('admin/members/' . $member_id, $format);
}

function memberful_admin_downloads_url($format = MEMBERFUL_HTML) {
    return memberfulSiteURL('admin/products', $format);
}

function memberful_admin_subscription_plans_url($format = MEMBERFUL_HTML) {
    return memberfulSiteURL('admin/subscriptions', $format);
}

function memberful_admin_download_url(
    $download_id,
    $format = MEMBERFUL_HTML
)
{
    return memberfulSiteURL('admin/downloads/' . (int) $download_id, $format);
}

function memberful_order_completed_url($order) {
    return add_query_arg('id', $order, memberfulSiteURL('orders/completed'));
}

function memberful_checkout_for_subscription_url($plan_id) {
    return add_query_arg('plan', $plan_id, memberfulSiteURL('checkout'));
}

function memberful_gift_url($plan_id) {
    return add_query_arg('plan', $plan_id, memberfulSiteURL('gift'));
}

function memberful_checkout_for_download_url($download_id) {
    return add_query_arg('download', $download_id, memberfulSiteURL('checkout'));
}

function memberful_wp_plugin_settings_url(
    $no_header = FALSE,
    $subpage=''
)
{
...
function memberful_wp_endpoint_url(
    $endpoint,
    $protocol = NULL
)
{
